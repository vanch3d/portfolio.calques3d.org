...
1. Verify the file exists in `.local/design-comps/<filename>`
...
6. Add or update the `**COMP:**` line in the surface brief's Direction Contract section:
   ...
   **COMP:** `.docs/design/comps/<filename>`
   ```
7. Report: confirmed path, surface brief updated, and remind the user that the approved comp is now the spec for the nextjs-engineer — it must not be edited; new changes require a new version number.
