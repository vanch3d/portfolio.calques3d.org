...
const { getAllPublications, getPublicationsByProject } =
  await import('../src/lib/api/zotero.ts').catch(async () =>
    // Fallback: run via tsx if available, otherwise report
    {
      console.error('Cannot import TS directly. Run: npx tsx scripts/test-zotero.mjs')
      process.exit(1)
    }
  )
