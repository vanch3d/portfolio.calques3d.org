...
if (withChildren.length > 0) {
  ...
  withChildren.forEach((i) =>
    console.log(
      `  [${i.key}] archiveLocation="${i.data.archiveLocation}" children=${i.meta.numChildren} — ${i.data.title.slice(0, 50)}`
    )
  )
}

console.log('\n=== archiveLocation → expected PDF filename ===')
withArchive.forEach((i) => console.log(`  ${i.data.archiveLocation}.pdf`))
