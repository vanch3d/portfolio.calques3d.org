...
export function PeriodStrip({ domain, ongoing = false, contextLabel }: PeriodStripProps) {
  ...
  const endColour = ongoing ? 'text-active' : 'text-ink-secondary'
  const endTickColour = ongoing ? 'bg-active' : 'bg-ink-secondary'
  ...
  return (
    ...
    >
      <span className="label text-ink-secondary tabular" data-testid="period-start">
        {domain.start}
      </span>
      ...
      <span className={cn('label tabular', endColour)} data-testid="period-end">
        {endLabel}
      </span>
    </div>
  )
