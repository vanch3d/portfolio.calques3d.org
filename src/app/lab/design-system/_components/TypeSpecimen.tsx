...
export function TypeSpecimen({
  ...
  return (
    <div
      className={cn(
        "grid gap-lg items-start pt-lg border-t-ghost border-ink-ghost",
    ...
    >
      ...
      <div className="flex flex-col gap-xs pt-[0.2em]">
        <p className="label mb-sm">{role}</p>
        {specs.map((spec) => (
          <p
            key={spec}
      ...
      <div className="flex flex-col gap-md">
        ...
        {rationale && (
          <p className="font-body text-caption leading-body text-ink-secondary max-w-prose">
            {rationale}
          </p>
        )}
      </div>
