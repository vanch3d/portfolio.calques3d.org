...
describe("PropsTable", () => {
  ...
  it("renders defaultValue when provided, dash when absent", () => {
    cy.mountAccessible(
      <PropsTable rows={[OPTIONAL_ROW, OPTIONAL_NO_DEFAULT]} />
    );
    cy.findByTestId("prop-row-initialActiveTags")
      .find("td")
      .eq(3)
      .should("contain.text", "[]");
    cy.findByTestId("prop-row-className")
      .find("td")
      .eq(3)
      .should("contain.text", "—");
  });
  ...
  it("has no axe accessibility violations", () => {
    cy.mountAccessible(
      <PropsTable rows={[REQUIRED_ROW, OPTIONAL_ROW, OPTIONAL_NO_DEFAULT]} />
    );
    cy.checkA11y();
  });
});
