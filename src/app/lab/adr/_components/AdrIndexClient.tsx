 ...
 */
export function AdrIndexClient({ adrs, tags, mostRecentAcceptedNumber }: AdrIndexClientProps) {
  ...
  const filteredAdrs = useMemo(() => {
    ...
    return adrs.filter((adr) => {
      ...
      const matchesTags =
        activeTags.length === 0 || activeTags.some((tag) => adr.tags.includes(tag))
  ...
  return (
    <div ref={containerRef}>
      ...
      <TagFilterDrawer tags={tags} activeTags={activeTags} onTagsChange={handleTagsChange} />

      <AdrRegisterTable adrs={visibleAdrs} mostRecentAcceptedNumber={mostRecentAcceptedNumber} />

      <div className="relative mt-lg flex items-center justify-center py-sm">
        <div
        ...
        <div
          className="absolute top-1/2 left-0 flex -translate-y-1/2 items-center gap-xs"
          aria-hidden="true"
        >
        ...
        <div
          className="absolute top-1/2 right-0 flex -translate-y-1/2 items-center gap-xs"
          aria-hidden="true"
        >
          <div className="h-sm border-l-medium border-ink-ghost" />
          <div className="h-sm border-l-medium border-ink-ghost" />
