...
describe("EraTimeline", () => {
  ...
  it("research column has a positions list", () => {
    cy.mountAccessible(<EraTimelineHarness />);
    cy.get("[data-testid='positions-list']").first().find("[role='listitem']").should("have.length.at.least", 3);
  });

  it("engineering column has a positions list", () => {
    cy.mountAccessible(<EraTimelineHarness />);
    cy.get("[data-testid='positions-list']").last().find("[role='listitem']").should("have.length.at.least", 2);
  });
