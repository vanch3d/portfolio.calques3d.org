...
const MOCK_ITEMS = [
  ...
  },
  {
    ...
    data: {
      ...
      creators: [{ creatorType: 'author', firstName: 'Nicolas', lastName: 'Van Labeke' }],
...
export const zoteroHandlers = [
  http.get(`${ZOTERO_BASE}/users/:userId/collections/:collectionId/items/top`, ({ request }) => {
    const url = new URL(request.url)
    const tag = url.searchParams.get('tag')

    const items = tag
      ? MOCK_ITEMS.filter((item) => item.data.tags.some((t) => t.tag === tag))
      : MOCK_ITEMS
