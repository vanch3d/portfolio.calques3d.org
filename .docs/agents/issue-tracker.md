...
Issues and specs for this repo live as GitHub issues on `vanch3d/portfolio.calques3d.org`. Use the `gh` CLI for all operations.
...
**PRs as a request surface: yes.** PRs run through the same labels and states as issues, using the `gh pr` equivalents:
...
- **Blocking**: GitHub's native issue dependencies. Where dependencies aren't available, fall back to a `Blocked by: #<n>, #<n>` line at the top of the child body. A ticket is unblocked when every blocker is closed.
